...
import com.minangdev.m_dosen.Adapter.BimbinganGroupChatAdapter
...
import kotlinx.android.synthetic.main.activity_bimbingan_detail_group.*
...
class BimbinganDetailGroupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_bimbingan_detail_group)
        ...
        respondBody.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.code() == 200) {
                    Toast.makeText(this@BimbinganDetailGroupActivity, "Berhasil Mengirim Pesan", Toast.LENGTH_SHORT).show()
                    tv_chat_detail_group_chat.setText("")
                } else if (response.code() == 422) {
                    Toast.makeText(this@BimbinganDetailGroupActivity, "Pesan Tidak Boleh Kosong", Toast.LENGTH_SHORT).show()
                } else {
    ...
    fun openImage(){
        Intent(Intent.ACTION_PICK).also {
            ...
            startActivityForResult(it, BimbinganDetailGroupActivity.REQUEST_CODE_IMAGE_PICKER)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(resultCode == Activity.RESULT_OK){
            when(requestCode){
                BimbinganDetailGroupActivity.REQUEST_CODE_IMAGE_PICKER -> {
